...
from data.pos_dataset_processor import *  
from data.cws_dataset_processor import *  
from models.bert_tagger import BertTagger 
from data.data_utils import convert_examples_to_features  
from utils.evaluate_funcs import compute_performance 
...
def train(model, optimizer, train_dataloader, dev_dataloader, test_dataloader, config, \
    ...
    for idx in range(int(config.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            loss = model(input_ids, segment_ids, input_mask, label_ids, loss_sign=config.loss_type,class_weight=class_w)
            if n_gpu > 1:
                loss = loss.mean()
