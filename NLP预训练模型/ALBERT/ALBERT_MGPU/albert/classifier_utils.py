...
class DataProcessor(object):
  ...
  def __init__(self, use_spm=True, do_lower_case=True):
...
class ZWTTProcessor(DataProcessor):
  """Processor for the ZWTT data set."""
  def get_train_examples(self, data_dir):
    """See base class."""
  ...
  def get_dev_examples(self, data_dir):
    ...
    for (i, line) in dev_data.iterrows():
      if i == 0:
        continue
      guid = "%s-%s" % (set_type, self.process_text(str(i)))
      text_a = self.process_text(str(line["result"]))
      text_b = self.process_text(line["fw"])
      if set_type == "test":
        label = "contradiction"
  ...
  def get_test_examples(self, data_dir):
    ...
    for (i, line) in test_data.iterrows():
      ...
      text_b = self.process_text(line["fw"])
      if set_type == "test":
        label = self.process_text(str(int(float(line["label"])))) #测试集label全部设置成0
      else:
        label = self.process_text(str(int(float(line["label"]))))
      examples.append(
          InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
